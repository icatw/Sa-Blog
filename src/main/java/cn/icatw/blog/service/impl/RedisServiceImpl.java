...
public class RedisServiceImpl implements RedisService {
    ...
    @Override
    public Long execute(RedisScript<Long> redisScript, ImmutableList<String> keys, int limitCount, int limitPeriod) {
        return redisTemplate.execute(redisScript, keys, limitCount, limitPeriod);
    }

    @Override
    public void likeOpt(Integer bizId, String prefixUserLike, String prefixLikeCount) {
        String bizLikeKey = prefixUserLike + UserUtil.getCurrentUser().getUserId();
        if (this.sIsMember(bizLikeKey, bizId)) {
            // 点过赞则删除评论id
            this.sRemove(bizLikeKey, bizId);
            // 评论点赞量-1
            this.hDecr(prefixLikeCount, bizId.toString(), 1L);
        } else {
            // 未点赞则增加评论id
            this.sAdd(bizLikeKey, bizId);
            // 评论点赞量+1
            this.hIncr(prefixLikeCount, bizId.toString(), 1L);
        }
